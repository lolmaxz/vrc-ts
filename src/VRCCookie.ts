...
export class cookiesHandler {
  ...
  async cookiesExist(username?: string): Promise<void> {
    try {
      ...
      if (username) {
        if (!username || !cookies[username] || (cookies[username] && cookies[username]?.length === 0)) throw new CookiesUser404("No Cookies was found for user '" + username + "'!");
      } else {
        if (!cookies[this.usernameOwner] || (cookies[this.usernameOwner] && cookies[this.usernameOwner]?.length === 0)) throw new CookiesUser404("No Cookies was found for user '" + this.usernameOwner + "'!");
      }
  ...
  public async addCookiesFromStrings(cookiesStrings: string[]): Promise<void> {
    ...
    cookies.forEach((cookie) => {
      ...
      if (index !== -1) {
        console.log(`✅ ${C.green + C.underline}Updated${C.r + C.green} cookie information: ${C.brightGreen + cookie.key} + Cookie Saving!${C.r}`);
        if (this.cookies[index].lastEmailCodeUsed) cookie.lastEmailCodeUsed = this.cookies[index].lastEmailCodeUsed;
        this.cookies[index] = cookie;
      } else {
        console.log(`✅ ${C.green + C.underline}Added${C.r + C.green} cookie information: ${C.brightGreen + cookie.key} + Cookie Saving!${C.r}`);
        this.cookies.push(cookie);
      }
    });
